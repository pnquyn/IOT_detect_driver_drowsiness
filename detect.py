...
class DrowsinessDetect:
    ...
    def detect_eyes_closed(self):
        ...
        if self.eye_is_closed == True and self.eye_alert == 0 and curr - self.eye_close_start >= eye_closed_time_bound_sec:
    ...
    def detect_drownsiness(self):
        frame = self.vs.read()
        frame = imutils.resize(frame, width=768, height=250)
        ...
        if (self.drowsiness_mouth != 0):
            drownsiness = 1
        elif (self.drowsiness_eye != 0):
            drownsiness = 2
        else:
            drownsiness = 0
        return frame, drownsiness, self.ear, self.mar
